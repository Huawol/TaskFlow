...
public enum ActivityType {
    ...
    public String description(ActivityLogCreateRequestDto requestDto) {
        return switch (this) {
            ...
            case USER_LOGGED_IN -> String.format("%s가 로그인했습니다.", requestDto.getUserId());
            case USER_LOGGED_OUT -> String.format("%s가 로그아웃했습니다.", requestDto.getUserId());
            case COMMENT_CREATED -> String.format("%s의 댓글이 작업되었습니다.", requestDto.getUserId());
            case COMMENT_UPDATED -> String.format("%s의 댓글이 수정되었습니다.", requestDto.getUserId());
            case COMMENT_DELETED -> String.format("%s의 댓글이 삭제되었습니다.", requestDto.getUserId());
