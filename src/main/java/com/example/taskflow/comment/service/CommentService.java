...
public class CommentService {
    ...
    public Long saveComment(Long postId, String content) {
        Task task = taskRepository.findById(postId).orElseThrow(
                () -> new IllegalArgumentException("Does not exist id = " + postId));

        User user = userRepository.findById(task.getUser().getId()).orElseThrow(
                () -> new IllegalArgumentException("Does not exist id = " + task.getUser().getId()));
    ...
    public void deleteCommentById(Long userId, Long commentId) { // hard delete
        ...
        Long postOwnerId = findComment.getTask().getUser().getId();
