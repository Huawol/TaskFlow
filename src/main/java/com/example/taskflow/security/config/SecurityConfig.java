	...
	public SecurityFilterChain filterChain(HttpSecurity httpSecurity) throws Exception {
		return httpSecurity
			...
			.authorizeHttpRequests(auth -> auth
				...
				.hasRole(UserRole.USER.name()) // 프로필 조회

				// 할일 인가
				.requestMatchers(HttpMethod.POST, "/api/tasks")
				.hasRole(UserRole.USER.name()) // 생성
				.requestMatchers(HttpMethod.GET, "/api/tasks/*")
				.hasRole(UserRole.USER.name()) // 단건 조회
				.requestMatchers(HttpMethod.GET, "/api/tasks")
				.hasRole(UserRole.USER.name()) // 전체 조회
				.requestMatchers(HttpMethod.PATCH, "/api/tasks/*")
				.hasRole(UserRole.USER.name()) // 상태값수정
				.requestMatchers(HttpMethod.PUT, "/api/tasks/*")
				.hasRole(UserRole.USER.name()) // 전체수정
				.requestMatchers(HttpMethod.DELETE, "/api/tasks/*")
				.hasRole(UserRole.USER.name()) // 삭제

				// 댓글 인가
				.requestMatchers(HttpMethod.POST, "/api/tasks/comments")
				.hasRole(UserRole.USER.name())
				.requestMatchers(HttpMethod.GET, "/api/tasks/comments/*")
				.hasRole(UserRole.USER.name())
				.requestMatchers(HttpMethod.GET, "/api/tasks/comments/*")
				.hasRole(UserRole.USER.name())
				.requestMatchers(HttpMethod.PATCH, "/api/tasks/comments/*")
				.hasRole(UserRole.USER.name())
			...
			.addFilterBefore(new JwtFilter(jwtUtil, tokenBlacklistRepository),
				UsernamePasswordAuthenticationFilter.class) // 토큰 검사
